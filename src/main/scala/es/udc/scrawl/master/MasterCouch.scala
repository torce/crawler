...
class MasterCouch(config: Config, listener: ActorRef) extends Actor with ActorLogging with MasterCouchViews {
  ...
  implicit val queryTimeout = config.getInt("scrawl.master.couch.query-timeout").milliseconds

  val userPass = try {
    Some((config.getString("scrawl.master.couch.user"), config.getString("scrawl.master.couch.pass")))
  } catch {
  ...
  val sprouchConfig = sprouch.Config(
    context.system,
    hostName = config.getString("scrawl.master.couch.hostname"),
    port = config.getInt("scrawl.master.couch.port"),
    userPass = userPass
  )

  val couch = Couch(sprouchConfig)
  val createDb = config.getBoolean("scrawl.master.couch.create-db")

  implicit val db = if (createDb)
    couch.createDb(config.getString("scrawl.master.couch.db-name")) recoverWith {
      case _ => couch.getDb(config.getString("scrawl.master.couch.db-name"))
    }
  else
    couch.getDb(config.getString("scrawl.master.couch.db-name"))
