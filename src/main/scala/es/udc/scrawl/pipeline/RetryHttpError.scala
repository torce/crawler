...
class RetryHttpError(config: Config) extends RequestFilter with ActorLogging {
  ...
  val toFilter: Set[StatusCode] = config.getIntList("scrawl.retry-http-error.errors").toSet[Integer]
    .map(i => StatusCode.int2StatusCode(i.intValue))
  val maxRetries: Int = config.getInt("scrawl.retry-http-error.max-retries")
