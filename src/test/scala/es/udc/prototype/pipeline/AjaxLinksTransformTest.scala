...
with BeforeAndAfterAll {
  ...
  "An AjaxLinksTransformer" should {
    "transform the url fragments into a escaped version" in {
      ...
      listener.send(ajax, new Response(new DefaultTask("id", Uri("http://www.example.com/"), 0), StatusCodes.OK, Map(), body))

      //Due to a bug, the TagSoup parser inserts an attribute shape="rect" into the a tags
      listener.expectMsgPF() {
        case Response(_, _, _,
        """<html><body><a href="http://www.example.com/?_escaped_fragment_=key%3Dvalue" shape="rect"/></body></html>""") => Unit
      }
    }
